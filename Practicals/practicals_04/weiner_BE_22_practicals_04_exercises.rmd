---
title: "Practicals 04: Gentle introduction to programming and R"
...
[https://forms.gle/XDVMAZhQ9qPKBmYQ8](https://forms.gle/XDVMAZhQ9qPKBmYQ8)


```{r}
library(qrcode)
plot(qr_code("https://forms.gle/XDVMAZhQ9qPKBmYQ8"))
```
...
This session is going to be a little bit different. You will observe what I
 ...
 * in Linux: pretty straightforward, install R (preferably >= 4.1) then R studio.
 * in Windows: 
   - install R (>= 4.1)
   - install R studio (rstudio.com)
   - install R tools
---
 ...
 * Go to `File` $\rightarrow$ `New File` $\rightarrow$ rmarkdown file
...
```{r pcaplot,eval=TRUE,echo=FALSE}
```

...
The `theme_set()` function returns an object which is a ggplot theme, and
...
else, e.g. `gear`? And what if you convert `gear` to a factor? And what if
...
```{r}
ggplot(iris, aes(x=Species, y=Sepal.Width)) + geom_violin() + 
  geom_jitter(alpha=.6, color="tomato", width=.1)
```
